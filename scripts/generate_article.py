...
- 具体的な数字や例を入れる（料金は必ず正確に記載。確認できない数字は書かない）
- 各セクションに1つ、読者が知らなさそうな具体的な情報を入れる（公式サイトや公式ブログで確認できる事実のみ。出典不明の「〜%」という数字は絶対に使わない）
...
## 禁止
- 嘘の情報・確信のない数字・古い料金情報
- 出典のない統計データ（「約70%の企業が〜」のような根拠不明の数字は絶対に書かない）
- 存在しない法律・制度・サービスの捏造（「2025年施行のAI規制法」のように実在しないものを書かない）
- 料金は公式サイトで確認できる正確な情報のみ記載（不明なら「公式サイトで要確認」と書く）
- 同じ内容の繰り返し・同語反復（「対策は〜することです。〜しましょう」のような中身のない文）
- 他サイトのコピペ感
- 「〜です。〜です。」の単調な繰り返し
- 1文が60文字を超える長文
- 「会話形式」と「対話形式」のような意味のない区別
- 内容の薄いセクション（各h2は最低150文字）
- 全セクションが同じ構造パターンになること（構成にバリエーションを持たせる）
...
本文（最低2500文字・目安3000〜3500文字。長さよりも具体性と正確性を優先）
...
## 記事の構成
1. 最新情報のポイントを3行で
2. 何が変わったのか詳しく（公式発表やリリースノートに基づく事実のみ）
3. ユーザーへの影響
4. 今後の予測（「予測」と明記し、事実と区別する）
5. 関連ツールまとめ（リンク付き）

## 重要な注意
- 実在する公式発表・リリース情報のみを記載すること
- 存在しない法律、架空の調査データ、未確認の情報を書かない
- 確認できない情報は「〜と報じられています」「〜の見通しです」と推測であることを明示
"""
